...
// 			got := VimmSearchRoms(tt.searchName, tt.filter)
//
// 			if len(got) == 0 {
...
// 			rom:  Rom{DownloadUrl: "https://dl3.vimm.net/?mediaId=4007"},
// 			path: "~/Downloads/test.7z",
// 		},
// 	}
// 	for _, tt := range tests {
// 		t.Run(tt.name, func(t *testing.T) {
// 			DownloadGame(tt.rom, tt.path)
// 		})
// 	}
// }
...
func Test_getCover(t *testing.T) {
	tests := []struct {
		...
		{
			name: "Test getCover",
			url:  "//dl.vimm.net/image.php?type=box_large&id=48075",
		},
	}

	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			img, err := getCover(tt.url)
			if err != nil {
